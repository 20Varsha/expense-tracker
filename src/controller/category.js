...
const create = async (req, res) => {
    try {
        ...
        if (result) { res.status(200).json({ message: constant.CATEGORY_CREATED, result: result }) }
...
const get = async (req, res) => {
    try {
        ...
        if (result) { res.status(200).json({ message: constant.FETCH_CATEGORY, count: result.length, data: result, error_code: 0 }) }
