...
const income_service = require("../service/income")
...
const create = async (req, res) => {
    try {
        ...
        if (result) { res.status(200).json({ message: constant.INCOME_CREATED, result: result }) }
...
const get = async (req, res) => {
    try {
        ...
        if (result) { res.status(200).json({ message: constant.FETCH_INCOME, count: result.length, data: result, error_code: 0 }) }
