...
const create = async (req, res) => {
    try {
        ...
        if (result) { res.status(200).json({status:"succes", message: constant.EXPENSE_CREATED, result: result }) }
...
const get = async (req, res) => {
    try {
        ...
        if (result) { res.status(200).json({ status:"succes",message: constant.FETCH_EXPENSE, count: result.length, data: result, error_code: 0 }) }
